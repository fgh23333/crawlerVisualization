  ...
  <div id="examPage">
    <topBar>
      ...
      <template v-slot:middle>
        <div class="paperTitle">{{ lesson }} - 试卷 {{ seq }}</div>
      </template>
    ...
    <ul>
      <li
        ...
        v-if="item.option.length == 4 && item.answer.length == 1"
        class="problemCover"
      >
    ...
    <ul>
      <li
        ...
        v-if="item.answer.length >= 2 && item.option.length > 2"
        class="problemCover"
      >
    ...
    <ul>
      <li
        ...
        v-if="item.answer == '正确' || item.answer == '错误'"
        class="problemCover"
      >
    ...
    <ul>
      ...
      >
        ...
        <span class="problem">
          <el-input
            v-model="input[l - 50]"
            placeholder="多空用中文逗号分隔，不要有多余的字符"
          ></el-input>
        </span>
      </li>
    </ul>
    <el-button type="text" @click="submitUserSelections()" class="submitButton"
      >提交</el-button
    >
  </div>
</template>
...
export default {
    ...
    submitUserSelections() {
      ...
      if (this.newSubject.includes(this.$route.params.lesson)) {
        ...
        if (!isNull) {
          ...
          for (let i = 20; i < 35; i++) {
            let result = arraysHaveSameElements(
              this.list[i].answer,
              this.userSelections[i]
            )
            if (result) {
              sum += 0.5
            } else {
  ...
  watch: {
    value: function (newval, oldval) {
      this.$router.push({
        path: '/newHome/examPage/' + this.$route.params.lesson + '/' + newval,
      })
