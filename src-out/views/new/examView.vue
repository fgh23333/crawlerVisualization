...
export default {
  ...
  methods: {
    ...
    getQuestion(lesson) {
      this.loadQuestionBank(
        require(`@/assets/cura/${lesson}_${this.$route.params.id}.json`)
      )
    },
    goBack() {
      this.$router.go(-1)
  ...
  watch: {
    value: function (newval, oldval) {
      this.$router.push({
        path: '/newHome/exam/' + this.$route.params.lesson + '/' + newval,
      })
    },
      ...
      if (to.params == from.params) {
        ...
        if (to.params.lesson === from.params.lesson) {
          if (to.params.id === 'random') {
            await this.getQuiz(
              this.newSubject.some(
                (element) => element == this.$route.params.lesson
              ),
          ...
          for (
            let i = 0;
            i < this.subjectList[this.$route.params.lesson].num;
            i++
          ) {
            let temp = {
              value: i + 1,
