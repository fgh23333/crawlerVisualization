  ...
  <div id="examCard">
    ...
    <div class="right">
      ...
      <div class="questionAnswer">
          ...
          >
            <div v-for="(item, i) in question.option" class="answer">
              <el-checkbox
                :label="options[i]"
                :key="i"
                class="option checkbox-item copyable-option"
              >
        ...
        <div
          class="single"
          v-if="questionType == '判断' || questionType == '单选'"
        >
          <div v-for="(item, i) in question.option" :key="i" class="answer">
            <el-radio
              v-model="radio"
              :disabled="examStatus"
              :label="question.option.length == 2 ? item : options[i]"
        ...
        <div class="fillingBlank" v-if="questionType == '填空'">
          <el-input
            class="copyable-option"
            v-model="input"
            :disabled="examStatus"
            @change="updateOption(seq, input)"
            placeholder="请输入答案"
          ></el-input>
        </div>
        <div class="trueAnswer" v-if="$store.state.answerList == ''">
          ...
          <span
            class="answerStatus true"
            v-if="
              $store.state.results[seq].questionId == question.id &&
              $store.state.results[seq].isCorrect
            "
            >回答正确</span
          >
          <span
            class="answerStatus false"
            v-if="
              $store.state.results[seq].questionId == question.id &&
              !$store.state.results[seq].isCorrect
            "
